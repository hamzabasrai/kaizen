...
interface HabitsState {
  ...
  toggleCompletion: (habitId: string, userId: string, date: string, completed: boolean) => Promise<void>;
  clearError: () => void;
}
    ...
    (set, get) => ({
      ...
      fetchHabit: async (id: string) => {
        ...
        try {
          ...
          set({ error: err instanceof Error ? err.message : 'Failed to fetch habit', isLoading: false });
        }
      },
      ...
      createHabit: async (habit) => {
        ...
        try {
          ...
          set({ error: err instanceof Error ? err.message : 'Failed to create habit', isLoading: false });
        }
      },
      ...
      updateHabit: async (id, updates) => {
        ...
        try {
          ...
          set((state) => ({
            ...
            currentHabit: state.currentHabit?.id === id 
              ? { ...state.currentHabit, ...updatedHabit } 
              : state.currentHabit,
            isLoading: false,
          }));
        } catch (err) {
          set({ error: err instanceof Error ? err.message : 'Failed to update habit', isLoading: false });
        }
      },
      ...
      toggleCompletion: async (habitId, userId, date, completed) => {
        try {
          await habitService.toggleHabitCompletion(habitId, userId, date, completed);
          ...
          set({ error: err instanceof Error ? err.message : 'Failed to toggle completion' });
        }
      },
